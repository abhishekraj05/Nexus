...
// components/FriendSystem/FriendsList.jsx
import React, { useState, useEffect } from "react";
import API from "../../api/api";
import "./FriendSystemView.css"; 
...
const FriendSystemView = () => {
  ...
  const getAvatarUrl = (friend) => {
    ...
    if (friend?.photoURL && !friend.photoURL.includes("DefaultUser")) {
  ...
  return (
    <div className="friend-list">
      <h4 className="friend-list__title">Your Friends ({friends.length})</h4>
      {message && <p className="friend-list__message">{message}</p>}
      
      <div className="friend-list__container">
        {friends.map((friend) => (
          <div key={friend._id} className="friend-list__item">
            <div
              className="friend-list__clickable-area"
            ...
            >
              ...
              <div className="friend-list__info">
                <span className="friend-list__name">{friend.name || 'Unknown User'}</span>
                <span className={`friend-list__status ${friend?.online ? 'friend-list__status--online' : 'friend-list__status--offline'}`}>
                  {friend?.online ? "Online" : "Offline"}
                </span>
              </div>
            </div>
            ...
            <button
              onClick={() => handleRemoveFriend(friend._id)}
              className="friend-list__remove-btn"
            >
              Remove
            </button>
          </div>
