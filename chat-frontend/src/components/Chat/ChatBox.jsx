...
// import MessageTicks from "./MessageTicks";         // Ticks component
// import MessageReactions from "./MessageReactions"; // Reactions display
// import EmojiPicker, { EmojiStyle } from 'emoji-picker-react'; // Emoji Picker
// import "./ChatBox.css";
// // --- 1. IMPORT date-fns ---
// import { format, isToday, isYesterday } from 'date-fns';
// // --------------------------

// const ChatBox = ({ chat, socket, onlineStatuses }) => {
//     // --- State ---
//     const [messages, setMessages] = useState([]); // Includes 'status' & 'reactions' Map
...
//     const chatSeenListenerRef = useRef(null);    // For Seen
...
//     // --- Format Last Seen Function (using date-fns) ---
//     const formatLastSeen = (lastSeenDateString) => {
//         if (!lastSeenDateString || typeof lastSeenDateString !== 'string' || status === true) {
//              return "Offline";
//         }
//         try {
...
//                 return `last seen today at ${format(lastSeenDate, 'p')}`; // 'p' = 10:30 PM
//             }
//             if (isYesterday(lastSeenDate)) {
//                 return `last seen yesterday at ${format(lastSeenDate, 'p')}`;
//             }
//             return `last seen on ${format(lastSeenDate, 'MMM d')}`; // Oct 24
...
//     };
//  //     // --- Handle Double Tap (Corrected Position) ---
// const handleDoubleTap = (event, messageId) => {
//     event.preventDefault();
//     const bubble = event.currentTarget; // The message bubble div
//     const rect = bubble.getBoundingClientRect();

//     // Calculate position relative to the viewport
//     let top = window.scrollY + rect.bottom + 5; // 5px below the bubble
//     let left = window.scrollX + rect.left;

//     // Optional: Prevent picker going off-screen (basic example)
//     const pickerHeight = 350; // Estimated height of picker
//     const pickerWidth = 300; // Estimated width
//     if (top + pickerHeight > window.innerHeight) {
//         top = window.scrollY + rect.top - pickerHeight - 5; // Position above
//     }
//     if (left + pickerWidth > window.innerWidth) {
//         left = window.innerWidth - pickerWidth - 10; // Adjust left
//     }
//     if (left < 10) {
//         left = 10; // Adjust left
//     }

//     // --- YEH DO LINES MISSING HAIN ---
//     setPickerPosition({ top: top, left: left }); // Step 1: Position set karo
//     setPickerVisibleFor(messageId);            // Step 2: Picker ko dikhao
//     // ---------------------------------
// };
...
//     // --- Main useEffect ---
// useEffect(() => {
//     // Guard clause: Exit early if chat or socket is not ready
//     if (!chat?._id || !socket) {
...
//                 });
//                 // Optimistically update local state (optional, but good)
...
//         }
//         setTypingUserId(null); setTypingUserName(""); // Clear typing state on cleanup
//     };

// // --- FIX: Removed typingUserId from dependency array ---
// }, [chat?._id, socket, user?._id, chat?.members]);
// // -----------------------------------------------------
  
//     // --- Auto-scroll & formatTime ---
...
//                       {typingUserId
//                         ? <span className="typing-indicator">{`${typingUserName || 'Someone'} is typing...`}</span>
//                         : status === true ? "Online" : formatLastSeen(status)}
//                     </span>
//                 </div>
//                  <div className="chat-header-icons"> <span>📞</span> <span>⋮</span> </div>
//             </div>
...
//                         return (
//                             <div key={msg._id} className={`message-wrapper ${isSender ? "sender" : "receiver"}`}>
//                                 <div
//                                     className="message-bubble"
//                                     onDoubleClick={(e) => handleDoubleTap(e, msg._id)} // <-- DOUBLE TAP HANDLER
//                                 >
//                                     {!isSender && chat.type === "group" && ( <strong className="sender-name">{msg.senderId?.name || "User"}</strong> )}
//                                     <span>{msg.type === "text" ? decryptMessage(msg.text) : msg.text}</span>
//                                     <div className="message-meta">
//                                         <span className="timestamp">{formatTime(msg.createdAt)}</span>
//                                         {isSender && <MessageTicks status={msg.status} />}
//                                     </div>
//                                     <MessageReactions reactionsMap={msg.reactions} /> {/* Display Reactions */}
//                                 </div>
//                             </div>
//                         );
//                     })
...
//              {/* Emoji Picker (Rendered Conditionally) */}
//              {pickerVisibleFor && (
//                 <div
//                     ref={pickerRef}
//                     className="emoji-picker-container" // Use this class for styling
//                     style={{ position: 'absolute', top: `${pickerPosition.top}px`, left: `${pickerPosition.left}px`, zIndex: 1100 }}
//                  >
//                     <EmojiPicker
//                         onEmojiClick={(emojiData) => onEmojiClick(emojiData, pickerVisibleFor)}
...
//                     setMessages((prev) => prev.some(m=>m._id===newMessage._id)?prev:[...prev, { ...newMessage, status: 'sent', reactions: new Map() }]);
//                  }}
//                 socket={socket}
//             />
